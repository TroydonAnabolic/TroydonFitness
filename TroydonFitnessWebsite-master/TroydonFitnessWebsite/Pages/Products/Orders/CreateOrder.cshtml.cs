        ...
        public async Task<IActionResult> OnPostAsync(int? id, [FromForm] WelcomeRequest request)
        {
            ...
            foreach (var item in CartItemsToOrder)
            {
                ...
                if (item.CartID != 0)
                {
                    ...
                    // TODO: Create a constants class, that associates a variable name with the branch path for each item e.g TrainingProductID = item.TrainingRoutine.PersonalTrainingSession.ProductID;
                    ...
                    if (item.TrainingRoutineID != null)
                    {
                        ...
                        // the commas are delimiters that will be used for splitting the built up string and then used to setup
                        // aappends a whole string, to be added to view, and then in the view we use conditional logic to add elements to be rendered into the view.
                        //sb.Append($"Order Item Number: ,{numberOfItemsInOrder},"); // uses the current iteration to set the order number item
                        //sb.Append($"Product ID: ,{item.TrainingRoutine.PersonalTrainingSession.ProductID},");
                        //sb.Append($"Training Routine ID: ,{item.TrainingRoutine.TrainingRoutineID},");
                        //sb.Append($"Product Name: ,{item.TrainingRoutine.PersonalTrainingSession.Product.Title},");
                        //sb.Append($"Price: ,{item.TrainingRoutine.PersonalTrainingSession.Product.Price},");
                        //sb.Append($"Quantity: ,{item.Quantity},");
                        //sb.Append($"Session Type: ,{item.TrainingRoutine.PersonalTrainingSession.PTSessionType},");
                        //sb.Append($"Length Of Routine: ,{item.TrainingRoutine.PersonalTrainingSession.LengthOfRoutine},");
                        //sb.Append($"Experience Level: ,{item.TrainingRoutine.PersonalTrainingSession.ExperienceLevel},");
                        //sb.Append($"Image: ,");
                        //var bytes = item.TrainingRoutine.PersonalTrainingSession.PtProductPicture;
                        ////foreach (var abyte in item.TrainingRoutine.PersonalTrainingSession.PtProductPicture)
                        ////{
                        ////    sb.Append(abyte);
                        ////}
                        //for (int i = 0; i < bytes.Length; i++)
                        //{
                        //    if (i == bytes.GetUpperBound(0)) sb.Append($"{bytes[i]},"); // when we reach the last byte, then we add a comma to break the array
                        //    else sb.Append(bytes[i]);
                        //}

                        // if its the last item on the list, we dont add a comma
                        //if (CartItemsToOrder.Count - 1 == numberOfItemsInOrder) sb.Append($"Comments: ,{item.TrainingRoutine.Comments}");
                        // otherwise we do
                        sb.Append($"Comments: ,{item.TrainingRoutine.Comments},"); // last one does not have a comma to avoid 40 items
                    }
                    else if (item.DietID != null)
                    {
                        OrderVM.TrainingRoutineID = item.Diet.DietID;
            ...
            OrderVM.OrderDetails = sb.ToString();
